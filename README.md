...
- 新增自动检查更新 — 应用启动时自动检测 GitHub 仓库最新版本，有新版本时顶部提示并可一键跳转下载

### v1.0.1 (2026-06-22)

**问题修复**

- 修复其他电脑上"无法确定当前账号"的问题 — 需要从 `.status.json` 的 `avatar_url` 字段提取 userId（需确保 QoderWork CN 为最新版本）
- 修复切换账号时覆盖已保存账号数据的问题 — 切换前自动保存当前账号会话
- 修复诊断信息面板显示异常（全红）的问题 — 修正 Rust 到 TypeScript 的字段名序列化

**功能改进**

- 新增额度加载骨架屏 — 应用启动时显示加载动画，不阻塞用户操作
- 新增诊断信息面板 — 点击顶部虫子图标查看 `.status.json` 检测状态，便于排查问题
- 优化刷新按钮响应 — 添加加载状态和重试延迟机制
